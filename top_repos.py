...
repository_id_Unique_list = []
repository_id_duplicate_list = []
...
for i in range(1,6):
    ...
    resp = requests.get(URI,headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.51 Safari/537.36'})
    if resp.status_code == 200:
        ...
        for item in items:
            ...
            if item['id'] not in repository_id_Unique_list:
                repository_id_Unique_list.append(item['id'])
                ...
                repository_id_duplicate_list.append(item['id'])
                db_session.commit()
